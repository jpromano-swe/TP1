...
bool imprimir_si_coincide_rareza(
    struct pokemon *pokemon,
    void *extra
)
{
...
int comando_buscar_nombre(tp1_t *tp1, const char *nombre)
{
    ...
    struct pokemon *pokemon =
        tp1_buscar_pokemon(tp1, nombre);

    if (pokemon == NULL) {
        fprintf(
            stderr,
            "No se encontro el pokemon %s\n",
            nombre
        );
